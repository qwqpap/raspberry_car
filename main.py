...
def line_finder(img=0):
    ...
    # ray_img = cv2.inRange(img, np.array([110, 0, 0]), np.array([140, 255, 255]))
    ...
    # djf = cv2.GaussianBlur(djf,(gs, gs), 1, 2)
    # qwq, djf = cv2.threshold(djf, 254, 255, cv2.THRESH_BINARY)
    # kernel = np.ones((4, 4), np.uint8)
    # djf = cv2.dilate(djf, kernel, 1)
    djf = cv2.GaussianBlur(djf, (gs, gs), 1, 2)
    djf = cv2.erode(djf, (5, 5), iterations=3)

    cv2.imshow("origin_pic", img)
    cv2.imshow("djff", gray_img)
    cv2.imshow("gs", djf)
